  ...
  tagList(
    bsModal(ns("modal_shp"), "Upload a shapefile to obtain well logs within it.", btn, size="large",
      ...
      tabsetPanel(
        ...
        tabPanel("Original shapefile", leafletOutput(ns("Map")), value="final"),
...
shpPoly <- function(input, output, session, r=NULL){
  ...
  output$Map <- renderLeaflet({ leaflet() %>% setView(lat = 36.7378, lng = -119.7871, zoom=5) %>% addTiles() })
  ...
  observe({
    if(!is.null(shp()) && tp()=="final"){
      cl <- class(shp_wgs84())
      x <- leafletProxy(ns("Map")) %>% clearShapes() %>% clearMarkers() %>% setView(lon(), lat(), zoom=9)
