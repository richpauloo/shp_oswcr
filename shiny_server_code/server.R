  ...
  output$Shp_Plot <- renderPlot({
    if(!is.null(shp())){
      ...
      if(cl=="SpatialPolygonsDataFrame"){
        ...
        if("hole" %in% names(d)) g <- g + geom_polygon(data=base::subset(d, hole==TRUE), fill="white")
  ...
  observe({
    if(!is.null(shp())){ 
      m <- leafletProxy("main_leaflet") %>%
        clearShapes() %>%
        addPolygons(data = shp()[[1]], fillOpacity = 0.1) %>%
        addMarkers(lng = shp()[[3]]@coords[, 1], lat = shp()[[3]]@coords[, 2],
                   clusterOptions = markerClusterOptions(),
                   popup = popupTable(shp()[[3]]@data, 
                                      zcol = c("WCRNumber","WCR_PDF","lat","lon","top","bot","type")
                                      )
                   )
      ...
      # 
        # addLayersControl(baseGroups = c("OpenStreetMap","Esri.WorldImagery","Esri.WorldTerrain", "CartoDB.DarkMatter"), #names(esri),
